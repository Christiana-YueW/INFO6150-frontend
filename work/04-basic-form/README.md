...
* Add the necessary files to have the required features
  ...
  - The `node_modules/` must NOT be added to your commit
    - Remember to check `git status` before committing, and to check the changes when creating the PR before cliclicking "Create PR".  These habits will save you time and embarrassment at work.
  - The `package-lock.json` file MAY be added to your commit, but is not required for this assignment.
...
- The "desktop" version must show a two-column form layout
...
- The page displays using `npm install`, `node server.js` as described
  - Errors from minor issues are only an Additional Requirement penalty.  If you aren't trying to do it correctly, that fails this requirement.
