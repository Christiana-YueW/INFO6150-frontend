...
- App should be a good UX from 360px wide to 1000px wide (when normal text size), including on mobile devices.  You choose what you need to do to make this a positive UX for those sizes (You can be just responsive if that works, or add adaptive breakpoints if that's the better option, all based on whether the result presents your content in a good experience for user)
...
- Use ARIA attributes when necessary to identify data or interactions
  - Not any ARIA you CAN do, the ones that are needed for your content, such as aria-expanded on the dropdown.  Do NOT include ARIA that isn't needed.
- All form fields must have correctly applied labels
- Keyboard navigation and mouse navigation must both work for all interactive elements
