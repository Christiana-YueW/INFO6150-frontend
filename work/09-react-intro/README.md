...
- Create a React project named "inventory" using Vite **inside this directory**
  - Follow the process shown in class to create your project, do NOT copy and start from the sample files
...
- The Reorder component is conditionally rendered
  - This means the HTML is or is not in the output.  Nothing should be hidden using CSS.
...
- Your PR does not include files it should not, even if related to the assignment (`.DS_Store`, `.idea`, `.vscode`, etc)
  ...
  - Exception: Files in `node_modules/` or `dist/` should NOT be included
...
- You followed the best practices listed in the course so far, including but not limited to (each a separate requirement):
  - HTML/JSX is valid and well-formed, containing all elements and details suggested from the course to date?
