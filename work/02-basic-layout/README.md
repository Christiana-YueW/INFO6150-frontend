...
- You are not expected to be "pixel-perfect", but have whitespace and color in the same places and roughly similar sizes.
- My sample images are a fixed size.  Your work is expected to fit the width of the screen.  If your browser window is taller than the content of the page, the content can all appear at the top of your browser viewport and there are no rules about what appears below the footer.
...
- Be sure to add padding/margins as needed to match the reference images
    ...
    - Most common mistake is to add a background color but no padding/margins, leaving the text feeling very "cramped" or "squished"
    - Another common issue is failing to get the padding to work with the `<a>` elements.  Remember that `<a>` elements are `display: inline;` by default, and that limits how padding/margin/etc can impact the text flow as shown in class if you don't change that display value.
        - Notice the cursor to click is active over the entire area with a background color!  This means if you hover over the area shown in a different color but are not yet hovering over the words of the link text, you see the hand cursor and can click there.  The area the link takes up is bigger than just the link text.

## Assignment Details

- Do NOT modify the HTML files
- Do NOT use `display: flex;`, `display: grid;`, `position`, or `float:`
...
- You obeyed all of the listed "Do NOT" under "Additional Restrictions"
  ...
  - In particular: Do NOT use `display: flex;`, `display: grid;`, `position`, or `float`
...
- You followed the best practices listed in the course so far, including but not limited to (each a separate requirement):
  ...
  - CSS selectors with just element types or combinations of types are setting defaults for that element type, not a specific appearance for a particular usage of that element type (Except where this assignment gives you no option)
